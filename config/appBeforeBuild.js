...
module.exports = (ctx) => {
  ...
  console.log("EXECUTING VERSION UPDATE");
  if (ctx.build && ctx.build.configuration && ctx.build.configuration === "production") {
    ...
    configXmlData = configXmlData.replace(/id="com.rigel.client-app" version=".*"/, `id="com.rigel.client-app" version="${packageJSON.version}"`);
